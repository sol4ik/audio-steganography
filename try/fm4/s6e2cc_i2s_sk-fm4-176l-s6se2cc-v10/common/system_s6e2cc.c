...
void SystemInit (void) {
  ...
  if ((uint8_t)(SCM_CTL_Val &  0xE0u)==(uint8_t)0x80ul) {    /* Low-speed CR oscillation selected ? */
    u8IoRegisterRead = (FM4_LCR->LCR_PRSLD & 0x3Fu);         /* Read LCR_PRSLD value */ 
    if(u8IoRegisterRead > (uint8_t)0ul) {
      uint32_t i;
      uint32_t u32WaitValue;
      u32WaitValue = (__CLKHC/__CLKLC)*u8IoRegisterRead;     /* caluculate wait value */
      FM4_LCR->LCR_PRSLD = (uint8_t)0x00u;                   /* Set "0" to LCR_PRSLD register */
  ...
  if( 0x000003FFu != (FM4_FLASH_IF->CRTRMM & 0x000003FFu) )
  {
    ...
    /* Set MCR_TTRM */
    FM4_CRTRIM->MCR_TTRM = (uint8_t)(FM4_FLASH_IF->CRTRMM >> 16);
    /* Set MCR_FTRM */
